...
export default function WhySection() {
  return (
    <section className="bg-muted py-24" id="about" aria-labelledby="why-heading">
      <Container>
        <div className="grid gap-10 lg:grid-cols-[1fr_1fr] lg:gap-16">
          <div>
            ...
            <div className="reveal delay-2 mt-6 border-t border-border">
              {reasons.map((reason) => (
                <div
                  key={reason.number}
                  className="flex gap-5 border-b border-border py-6 transition-all hover:pl-3"
                >
          ...
          <div className="reveal-right delay-2">
            <div
              className="rounded-brand-xl border border-border bg-white p-6 shadow-brand-md sm:p-8 lg:p-10"
              data-skill-block
            >
              <div className="font-display text-[15px] font-bold text-muted-foreground">
                NumerLett Capability Depth
              </div>
              <div className="mt-7 space-y-4">
                {skills.map((skill) => (
                  <div key={skill.label} className="flex items-center gap-3">
                    <span className="min-w-25 text-[13px] font-semibold text-muted-foreground">
                      {skill.label}
                    </span>
                    ...
                    <span className="min-w-8 text-right font-mono text-[12px] font-bold text-primary">
                      {skill.value}
                    </span>
                  </div>
                ))}
